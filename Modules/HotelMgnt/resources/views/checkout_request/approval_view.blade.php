@php use Modules\HotelMgnt\Models\CheckOutRequest; @endphp
...
@section('content')
        ...
        <div class="ibox-body">
            ...
            <div class="row">
                <div class="col-9" style="padding-top: 2vh">
                    <h5 class="font-strong">CHECKOUT REQUEST</h5>
                </div>
                ...
                <table class="table table-bordered table-hover table-sm" id="datatable">
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td style="text-align: right">{{number_format(CheckOutRequest::totalCharges($item->booking_id))}}</td>
                                ...
                                @if($item->is_approved ==null)
                                    @can('Manager')
                                        <a class="text-muted font-16 review-link"
                                           href="{{route('checkout-req.review',$item->id)}}"
                                    ...
                                    @can('Director')
                                        <a class="text-muted font-16 approve-link"
                                           href="{{route('checkout-req.approve',$item->id)}}"
                                           title="Approve" data-toggle="tooltip"><i
                                                    class="fa fx-2 fa-check-circle-o"></i></a>
                                    @endcan

                                    | <a class="text-muted font-16 reject-link"
                                         href="{{route('checkout-req.reject-view',$item->id)}}"
                                         title="Reject" data-toggle="tooltip"><i
                                                class="fa fx-2 fa-close"></i></a>

                                @endif
                                | <a class="text-muted font-16 charges-link"
                                     href="{{route('booking-charges.show',$item->booking_id)}}"
                                     title="Booking Charges" data-toggle="tooltip"><i
                                            class="fa fx-2 fa-eye"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    <!-- Preview Booking Charges-->
    <div class="modal fade" id="bk_charges" aria-labelledby="bk_charges" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content modal-bk-charges">
                <!--Edit Form Loads Here-->
            </div>
        </div>
    </div>
...
@section('Scripts')
    <script>
        $('.charges-link').on('click', function (e) {
            e.preventDefault();
            const dataURL = $(this).attr('href');
            $('.modal-bk-charges').load(dataURL, function () {
                $('#bk_charges').modal({show: true});
            });
        });
